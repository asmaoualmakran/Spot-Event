...
class Event(serializers.HyperlinkedModelSerializer):
	...
	likedBy = serializers.PrimaryKeyRelatedField(many=True, read_only=True)

	class Meta: 
		model = eventModel
		fields = ('id','venue_id','user_id','event_name','event_date','artists','genre', 'upload', 'likedBy')
...
class Create_event(serializers.ModelSerializer):
	...
	class Meta: 
		model = eventModel
		fields = ('venue_id','user_id','event_name','event_date','artists','genre', 'upload', 'likedBy')
